namespace BTree
{
    ...
    public class BTree<TK, TP> where TK : IComparable<TK> where TP : List<int>
    {
        ...
        private void Update(TK keyToUpdate, TP newPointer)
        {

            var entry = Search(keyToUpdate);
            if (entry.Pointer.GetType().IsArray == newPointer.GetType().IsArray)
            {
                entry.Pointer = entry.Pointer.Union(newPointer).OrderBy(a => a).ToList();
        ...
        private void DeleteKeyFromNode(Node<TK, TP> node, TK keyToDelete, int keyIndexInNode)
        {
            ...
            {
                Entry<TK> predecessor = this.DeletePredecessor(predecessorChild);
        ...
        private Entry<TK> DeletePredecessor(Node<TK, TP> node)
        {
            if (node.IsLeaf)
            {
        ...
        private Entry<TK> DeleteSuccessor(Node<TK, TP> node)
        {
            if (node.IsLeaf)
            {
        ...
        private Entry<TK> SearchInternal(Node<TK, TP> node, TK key)
        {
        ...
        private void InsertNonFull(Node<TK, TP> node, TK newKey, TP newPointer)
        {
            ...
            if(node.Entries.Any(entry=>newKey.Equals(entry.Key)))
            {
                var updatingEntry = node.Entries.FirstOrDefault(entry => newKey.Equals(entry.Key));
                Update(updatingEntry.Key, newPointer);               
                return;
            }
            ...
            if (node.IsLeaf)
            {
                node.Entries.Insert(positionToInsert, new Entry<TK>() { Key = newKey, Pointer = newPointer });
                return;
            }
        ...
        public override string ToString()
        {
            ...
            sb.AppendLine(string.Format("Root: {0} {1}",Environment.NewLine, Root.ToString()));
            return sb.ToString();
        }
    }
}
